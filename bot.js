...
var names = [];
var permroles = ["owner","admin","Owners"];
var gja;
var warns = [];
var mco = [];
...
client.on("message", (msg, member) => {
  ...
  if (msg.content.toLowerCase().startsWith("lunio, message")) {
    msg.delete();
  ...
  if (msg.content.toLowerCase().startsWith("lunio, delete") || msg.content.toLowerCase().startsWith("lunio, clear")) {
    msg.delete();
    var mc = parseInt(msg.content.split("lunio, delete")[1]);
    if (mc > 1) {
  ...
  if (msg.content.toLowerCase().startsWith("lunio, warn ")) {
  msg.delete();
  ...
  if (msg.content.toLowerCase().startsWith("lunio, mute ")) {
    msg.delete();
    msg.mentions.members.first().addRole(msg.guild.roles.find("name", "muted").id);
    return;
  }

  if (msg.content.toLowerCase().startsWith("lunio, unmute ")) {
    msg.delete();
  ...
  if (msg.content.toLowerCase().startsWith("lunio, kick ")) {
  msg.delete();
  var member = msg.mentions.members.first();
...
if (msg.content.toLowerCase().startsWith("lunio, ban ")) {
  msg.delete();
  var member = msg.mentions.members.first();
...
if (msg.content.toLowerCase().startsWith("lunio, unban ")) {
  msg.delete();
  msg.guild.unban(msg.content.split("lunio, unban ")[1])
...
return;
  ...
  if (msg.content.toLowerCase().startsWith("lunio, addrole ") && msg.member.roles.some(r=>("Owner").includes(r.name))) {
    msg.delete();
    var nr = msg.content.toLowerCase().split("lunio, addrole ")[1];
    permroles.push(nr);
  ...
  } else if (msg.content.toLowerCase().startsWith("lunio, ")) {
  msg.delete();
  msg.reply("You dont have access to this command!");
  console.log(chalk.grey.bgCyan.bold("Daddy! " + msg.author.username + " tried to force me to do things i didn't want! He asked me to " + msg.content.toLowerCase().split("lunio, ")[1] + "!"));
  return;
}
  ...
  if (msg.content.toLowerCase() === "what are the commands" || msg.content.toLowerCase() === "help") {
    msg.delete();
    var listit = ["say", "who made lunio", "calc", "translate", "ban", "unban", "kick", "ping", "rank", "listen!", "clear chat", "addrole", "message", "mute", "unmute", "compliment me", "requestcompliment", "clantag", "help", "setexp", "addexp", "removeexp", "setlvl", "addlvl", "removelvl", "shutdown"];
...
client.on("guildMemberRemove", member => {
  if (member.user.username === client.user) return;
  dates = [];
  date = new Date();
  ...
  client.channels.find("name","welcome").send("R.I.P " + member.user.username + " : " + dates.join(" "));
  console.log(chalk.grey.bgCyan.bold(member.user.username + " left our server, Daddy!"));
});
client.on("guildMemberAdd", member => {
  if (member.user.username === client.user) return;
  client.channels.find("name","welcome").send("Welcome to LuneXum, " + member.user.username + "! To get the command list just say what are the commands !");
